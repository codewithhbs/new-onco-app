...
// 🔹 Helper: coupon discount calculation
const calculateCouponDiscount = (coupon, baseAmount) => {
  if (!coupon || !baseAmount || baseAmount <= 0) return 0

  const minOrder = Number(coupon.min_order_value || 0)
  if (baseAmount < minOrder) {
    // order value kam hai → coupon apply nahi hoga
    return 0
  }

  const off = Number(coupon.percenatge_off || 0)

  // FIXED discount → direct amount minus
  if (coupon.discount_type === "Fixed") {
    return Math.min(off, baseAmount) // total se zyada discount nahi
  }

  // PERCENTAGE discount
  if (coupon.discount_type === "Percentage") {
    let discount = (baseAmount * off) / 100

    // optional cap: maxDiscount agar diya ho
...
const PriceBreakdown = ({ items, coupons, enabledCheckOutButton = false, logged }) => {
  ...
  // OncoHealthMart discounted total
  const oncoDiscount = items.reduce((sum, item) => sum + item.Pricing * item.quantity, 0)

  const totalWithoutDiscount = subtotal

  // Base amount jispe coupon lagana hai (tum chaaho to subtotal bhi use kar sakte ho)
  const baseAmountForCoupon = oncoDiscount

  // Coupon discount according to discount_type (Fixed / Percentage)
  const discountAmount = appliedCoupon
    ? calculateCouponDiscount(appliedCoupon, baseAmountForCoupon)
    : 0

  // Delivery fee logic
  const deliveryFee =
    oncoDiscount > (settings?.shipping_threshold || 0)
      ? 0
      : Number(settings?.shipping_charge || 0)

  // Final total (negative nahi hone denge)
  const grandTotal = Math.max(oncoDiscount + deliveryFee - discountAmount, 0)
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      const appliedCode = coupons?.CODE?.trim() || couponCode.trim()

      // Backend se validation (expiry, usage limit, etc.)
      ...
      if (data.success) {
        // Yaha hum apna coupon object use kar rahe hain (props se ya response se)
        const finalCoupon = coupons
          ? { ...coupons, code: appliedCode }
          : {
              code: appliedCode,
              discount_type: data.discount_type,
              percenatge_off: data.percenatge_off,
      ...
      setError(
        "We're sorry, but this coupon code appears to be invalid. Please check the details and try again"
      )
      animateShake()
      console.error("Coupon application error:", error?.response?.data || error)
      setAppliedCoupon(null)
    } finally {
      setLoading(false)
    }
  ...
  // 🔹 Auto-apply coupon jab coupons?.CODE change ho
  useEffect(() => {
    if (coupons?.CODE) {
      handleApplyCoupon()
    } else {
  ...
  const renderPriceRow = (label, value, type = "regular", icon = null) => (
    <View style={styles.priceRow}>
      <View style={styles.priceRowLeft}>
        {icon && (
          <Icon
            name={icon}
            size={16}
            color={type === "discount" ? "#4CAF50" : "#555"}
      ...
      <Text
        style={[
          styles.priceValue,
          type === "total" && styles.totalValue,
          type === "discount" && styles.discountValue,
        ]}
      >
        {value}
      </Text>
      ...
      <ScrollView style={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        <View style={styles.savingsCard}>
          ...
          <Text style={[styles.savingsText, { fontSize: 11 }]}>
            🎉 Enjoy <Text style={styles.savingsAmount}>FREE Delivery</Text> on orders above ₹
        ...
        {totalSavings > 0 && (
          <View style={styles.savingsCard}>
            ...
            <Text style={styles.savingsText}>
              You're saving{" "}
              <Text style={styles.savingsAmount}>₹{totalSavings.toLocaleString()}</Text> on this
              order!
            </Text>
          </View>
        )}
              ...
              <View style={styles.checkoutContent}>
                <View style={styles.checkoutLeft}>
                  <Text
                    style={[
                      ...
                      !enabledCheckOutButton && styles.disabledCheckoutText,
                    ]}
                  >
                    ₹{grandTotal.toLocaleString()}
                  </Text>
                ...
                <View style={styles.checkoutRight}>
                  ...
                  <Icon
                    name="chevron-right"
                    size={20}
                    color={enabledCheckOutButton ? "#fff" : "#999"}
                  />
                </View>
              </View>
            </LinearGradient>
